...
const Leaderboard = memo(() => {
  ...
  const [leaderboardData, setLeaderboardData] = useState<LeaderboardUser[]>(MOCK_LEADERBOARD);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setIsLoading(true);
        const users = await usersApi.getAll({ limit: 10 });
        const mapped = users.map((u, i) => ({
          rank: i + 1,
          name: u.name,
          handle: u.handle,
          profit: (u.sosTokenBalance || 0) * (Math.random() > 0.3 ? 1 : -0.5), // Mock PnL from balance
          winRate: Math.floor(40 + Math.random() * 45),
          avatar: u.avatar || 'https://picsum.photos/200',
        }));
        setLeaderboardData(mapped.length > 0 ? mapped : MOCK_LEADERBOARD);
      } catch (error) {
